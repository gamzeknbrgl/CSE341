...
(defun insert-n	(list x index)
;When the index we search for is found we can add the
;elements we want and merge the list with the other half.
(if  (= 0 index) 
(cons x list)
(cons (car list) (insert-n (cdr list) x (1- index) ))
))
